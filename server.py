...
def evaluate_fn(roundn, parameters, config):
    net = net = Baseline_net(3, "unet", "brain").to(DEVICE).to(DEVICE)
...
class Custom_straegy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Tuple[ClientProxy, FitRes] | BaseException]) -> Tuple[Parameters | None, Dict[str, Scalar]]:
        ...
        if aggregated_parameters is not None:
            ...
            # Save the model
            torch.save(self.net.state_dict(), f"./models/Federated_brain_unet/model_round_{server_round}.pth")
...
if __name__ == "__main__":
    ...
    history=fl.server.start_server(
        ...
        strategy=Custom_straegy(min_fit_clients=4, min_available_clients=4, min_evaluate_clients=4, evaluate_fn=evaluate_fn),
    ...
    csv_history={
        ...
        "acc" : history.metrics_centralized["acc"],
        "mIOU": history.metrics_centralized["mIOU"]
    }
    save_csv(csv_history, mode="train", name="Brain_FL")
