...
def aggregate(results: List[Tuple[NDArrays, int]], client_lis, round) -> NDArrays:
    ...
    if round>2:
        ...
        weighted_dice_layer=[
         [layer*loss for layer in weights] for (weights, _), loss in zip(results, client_iou_PIDAVG(client_lis, client_num))
        ]
    ...
    # Compute average weights of each layer
        weights_prime: NDArrays = [
            ...
            +(0.25*reduce(np.add, loss_layer)/global_loss_PIDAVG(client_lis, client_num))
            +np.nan_to_num((0.25*reduce(np.add, dice_layer)/global_iou_PIDAVG(client_lis, client_num)))
            for layer_updates, loss_layer, dice_layer in zip(zip(*weighted_weights), zip(*weighted_loss_layer), zip(*weighted_dice_layer))
        ]
...
if __name__ == "__main__":
    ...
    save_csv(csv_history, mode="train", name="Brain_FL_Prox_proposed")
